...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof patch.technologies !== "undefined") {
    patch.technologies = Array.isArray(patch.technologies) ? patch.technologies : []
  }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  const { error } = await supabaseAdmin.from("work_experience").delete().eq("id", id).eq("candidate_id", candidateId)
